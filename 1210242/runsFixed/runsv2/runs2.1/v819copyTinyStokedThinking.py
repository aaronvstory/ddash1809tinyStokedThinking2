	...
	# --- Circuit Breaker ---
	driver_breaker = CircuitBreaker(fail_max=3, reset_timeout=30)
	support_chat_breaker = CircuitBreaker(fail_max=3, reset_timeout=30)
	send_message_breaker = CircuitBreaker(fail_max=3, reset_timeout=30)
	...
	def log_error(message: str) -> None:
			...
			console.print(f"❌ [red bold] { message }[/]", style="error")
			console.print()
	...
	def log_info(message: str, transient: bool = True) -> None:
			...
			logging.info(message)
			if transient:
					console.print(f"ℹ️ [blue bold] { message }[/]", end="\r")
			else:
					console.print(f"ℹ️ [blue bold] { message }[/]", style="info")


	def log_warning(message: str) -> None:
			"""Log warning messages with proper spacing."""
			logging.warning(message)
			console.print(f"⚠️ [yellow bold] { message }[/]", style="warning")
			console.print()
					...
					try:
							...
							for attempt in range(MAX_RETRIES):
									try:
											...
											log_success(f"Clicked 'It's something else' for order: {shorten_order_id(self.id)}", transient=True)
											break
							...
							for attempt in range(MAX_RETRIES):
									try:
											...
											log_success(f"Clicked 'Contact support' for order: {shorten_order_id(self.id)}", transient=True)
											break
									except (TimeoutException, StaleElementReferenceException) as e:
											log_warning(f"Attempt {attempt + 1} failed: {str(e)}")
											if attempt < MAX_RETRIES - 1:
	...
	class SupportAgent:
			__slots__ = ['AGENT_CHECK_INTERVAL', 'MAX_AGENT_CHECKS', 'AGENT_NAME_PATTERN', 'AGENT_MESSAGE_TEMPLATE', 'MAX_SEND_RETRIES', 'SEND_RETRY_DELAY']
	...
	def process_orders(elements: List[uc.WebElement]) -> List[Order]:
			...
			with Progress(
					...
					log_info(f"Eligible for processing: {len(orders)}", transient=False)
					console.print("")
	...
	def print_order_status(order, current, total):
			...
			status_text.append(f" Found eligible order ({current}/{total}): ", style="white")
			status_text.append(f"Order #{sh

Tokens: 113871 (prompt: 94553, completion: 19318)
			...
			status_text.append("✅" if not order.cancelled else "❌", style="green bold")
	...
	class OrderStats:
			...
			def print_summary(self):
					...
					summary_table = Table(title="[bold]Order Summary[/bold]", title_style=f"bold {MAIN_COLOR}", border_style=MAIN_COLOR, padding=(1, 2))
	...
	class DriverContextManager:
			"""Context manager for WebDriver to handle setup and teardown."""
			def __enter__(self):
					log_info("Setting up WebDriver...", transient=False)
					snapshot_memory("Before driver creation")
					self.driver = create_driver()
					self.driver.implicitly_wait(IMPLICIT_WAIT)
					self.start_time = time.perf_counter() # Start timing for the session
			...
			def __exit__(self, exc_type, exc_val, exc_tb):
					...
					if _memory_snapshots:
							memory_table = Table(title="[bold]Memory Usage Snapshots[/bold]", title_style=f"bold {MAIN_COLOR}", border_style=MAIN_COLOR, padding=(1, 2))
							...
							session_start_time = _memory_snapshots[0][2] if _memory_snapshots else self.start_time # Use session start time if snapshots exist, else context manager start time
